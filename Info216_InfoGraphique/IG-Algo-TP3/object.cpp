	...
	for(unsigned int i=0; i<faces.size(); i++)
	{
		switch(draw_method)
		{
			...
			case DRAW_FILL :
			    {
			        vec2 p[4];
			        p[0] = vertices_projected[faces[i].vertex_index[0]];
			        p[1] = vertices_projected[faces[i].vertex_index[1]];
			        p[2] = vertices_projected[faces[i].vertex_index[2]];
			        p[3] = vertices_projected[faces[i].vertex_index[3]];
