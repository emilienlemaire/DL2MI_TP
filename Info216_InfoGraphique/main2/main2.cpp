...
int main(){
	...
	GLuint programID = LoadShaders( "SimpleVertexShader2.vertexshader", "SimpleFragmentShader2.fragmentshader" );
	...
	do{
        ...
        GLfloat g_vertex_buffer_data_updated[18];

        if(height >= width){
            ratio = ((float)width/(float)height);

            GLfloat otherArray[] = {
                    -0.8f, - 0.8f * 1 / ratio, 0.0f,
                    0.8f, - 0.8f * 1 / ratio, 0.0f,
                    -0.8f,   0.8f * 1 / ratio, 0.0f,
            ...
            for (int i = 0; i < sizeof(g_vertex_buffer_data_updated); ++i) {
                g_vertex_buffer_data_updated[i] = otherArray[i];
            }

        } else {
            ratio = ((float)height/(float)width) / 2;

            GLfloat otherArray[] = {
                    -0.8f * 1 / ratio, - 0.8f, 0.0f,
                    0.8f * 1 / ratio, - 0.8f, 0.0f,
                    -0.8f * 1 / ratio,   0.8f, 0.0f,
        ...
        glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
          ...
          // send vertices in the first part of the buffer
          glBufferSubData(GL_ARRAY_BUFFER, 0,                            sizeof(g_vertex_buffer_data), g_vertex_buffer_data_updated);
