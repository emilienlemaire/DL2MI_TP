...
public class OnlinePerceptron {
     ...
     */
    public static void miseAJour(float[]w, float[] data, int classe) throws IOException {
        for (int i = 0; i < w.length; i++) {
            if (i == 0){
                w[i] += classe;
            } else {
                w[i] = w[i] + (classe * data[i]);
            }
        }
     ...
     */
    public static float[] epoque(float[] w, float[][] data, int[] refs) throws IOException {
     ...
     */
    public static void apprentissage(float[] w, float[][] data, int[] refs, int T) throws IOException {
        for (int i = 0; i < T; i++) {
            epoque(w, data, refs);
        }
